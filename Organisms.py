...
import random
'''
Придумать систему классов, реализующих, например, геометрические фигуры (фигура,
квадрат, прямоугольник) или алгебраические типы (число, вектор, матрица), или 
что-то своё. Придумать, как поинтересней перегрузить стандартные операции,
использовать полиморфизм). Все действия должны быть покрыты тестами.
'''
class Organism:
    def __init__(self, mut_dict: Dict = {}, ):
        ...
        self.can_swim = False
        self.mutations = {}
        """ If mutations['name'] is True, it is strong and transits to the child.
            The weak mutation transits to child only from both parents having 
            mutations['name'] and it will be weak mutation too.
        """
    ...
    """ In my world this means breeding. Can be applied to mother object with 
        father as an argument. If all was done right, returns a new organism of the
        same type, else returns None.
    """
...
# read comment in __init__        
        ...
        for mut, strength in enumerate(self.mutations):
            if strength:
                mutations[mut] = True
...
class Fish(Organism):
    ...
    def __sub__(self, eater):
        if not eater.can_swim() and self.swim():
            eater.take_injury()
            return False
            
        Organism.__sub__(self)
...
class Bird(Organism):
    ...
    def __sub__(self, eater):
        if not eater.can_fly() and self.fly() and self.is_alive:
            eater.take_injury()
            return False
